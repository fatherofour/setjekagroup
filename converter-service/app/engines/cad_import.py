...
# Copyright (c) 2026 Setjeka Group
...
def _deb_arch_tag() -> str:
    ...
    Used so ``find_converter`` can locate the per-arch directory a
    previously-installed Linux converter lives under.
    """
...
CONVERTER_SEARCH_PATHS: list[Path] = [
    ...
    Path("C:/ProgramData/Setjeka Group/converters"),
]
...
# ── Converter provisioning (vendored, no network) ──────────────────────────
#
# Why this exists: ``find_converter`` only LOCATES an already-present
# binary. ``ensure_converter`` used to also auto-download a missing
# converter from the upstream repository on first use. That auto-download
# path has been removed: the converter binaries are now vendored with this
# application (installed once under ``~/.openestimator/converters/``) and
# are never fetched from - or checked against - the original upstream repo
# again. ``ensure_converter`` now does exactly one thing beyond
# ``find_converter``: if the binary is missing, it raises a clear,
# actionable :class:`ConverterUnavailableError` telling the caller to
# contact support, instead of reaching out to the network.
...
def ensure_converter(fmt: str) -> Path:
    """Resolve a CAD/BIM converter that is already vendored on this machine.
    ...
    2. If it is not present, this raises :class:`ConverterUnavailableError`
       with a clear, actionable message. Converter binaries ship bundled
       with this application; they are never downloaded from - or checked
       against - the upstream repository automatically. An operator must
       install the missing converter package out of band (contact support).
    ...
    Raises:
        ConverterUnavailableError: The converter is missing (unsupported
            platform, unknown format, or simply not installed on this
            machine). The message is user-actionable.
    """
    ...
    if sys.platform == "darwin":
        raise ConverterUnavailableError(
            ...
            f"Run Setjeka Group in Docker (Linux container - the converter "
    ...
    # Converters are vendored with this application (installed once under
    # ~/.openestimator/converters/) and are no longer auto-downloaded from
    # the upstream repository on first use. ``find_converter`` above is the
    # only lookup that ever happens - if it didn't resolve a binary, there
    # is nothing further this function can do except say so clearly.
    logger.info("Converter for .%s not found and auto-download is disabled - not installed on this machine", fmt)
    raise ConverterUnavailableError(
        f"The .{fmt.upper()} converter is not installed on this machine. "
        f"Converters ship bundled with this application and are not "
        f"downloaded automatically - please contact support to obtain the "
        f"{fmt.upper()} converter package."
    )
...
async def ensure_converter_async(fmt: str) -> Path:
    ...
    ``ensure_converter`` does blocking disk IO (a file-stat lookup), so it
...
def smoke_test_converter(extension: str, force: bool = False) -> ConverterHealth:
    ...
    if exe_path is None:
        result: ConverterHealth = {
            "status": "failed",
            "message": (
                f"The .{extension.upper()} converter is not installed on this "
                f"machine. Converters ship bundled with this application - "
                f"contact support to obtain the {extension.upper()} converter package."
            ),
...
async def convert_cad_to_excel(
    ...
    from .dxf_native import convert_dxf_to_excel, is_natively_readable

    if is_natively_readable(input_path.suffix):
        return await asyncio.to_thread(convert_dxf_to_excel, input_path, output_dir)

    # Resolve the already-vendored converter. ``ensure_converter`` never
    # downloads anything; on an unsupported platform (or a converter that
    # simply isn't installed on this machine) it raises
...
def _ddc_cad2data_verify() -> bool:
    """Setjeka Group CAD2DATA pipeline verification. DDC-CWICR-2026."""
